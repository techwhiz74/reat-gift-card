...
import { CanvasContext } from "../GiftCardCanvas.js";
import { resizeHandleClasses } from "../canvasUtils.js";
import GiftTextElement from "./GiftTextElement.js";
import { useMediaQuery } from "@mui/material";

const componentMap = {
  TEXT: GiftTextElement,
};
...
export default function GiftSubCanvasComponent(props) {
  ...
  const onBlur = (event) => {
    ...
    if (id && activeSelection) {
      ...
      actions?.setActiveSubSelection(new Set(activeSelection));
    }
  };
  ...
  const onfocus = (event) => {
    if (id) {
      actions?.setActiveSubSelection(new Set(state?.activeSelection.add(id)));
    }
  };
